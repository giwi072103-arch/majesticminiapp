...
// Универсальный рендер: находит первое поле-массив в объекте
// (vehicleStatistics, itemStatistics и т.п.) -> карточки,
// остальные скалярные поля объекта -> строка метаданных сверху.
function renderData(data) {
  const wrapper = document.createElement('div');

  if (Array.isArray(data)) {
    wrapper.appendChild(renderCards(data));
    return wrapper;
  }
  ...
  if (data && typeof data === 'object') {
    ...
    if (arrayEntry) {
      wrapper.appendChild(renderCards(arrayEntry[1]));
      return wrapper;
    }
  }

  // Ничего похожего на массив не нашли — просто дампим JSON
...
function renderMeta(scalarEntries) {
  ...
  meta.innerHTML = scalarEntries
    .map(([k, v]) => `<span class="meta-chip"><b>${escapeHtml(k)}:</b> ${escapeHtml(formatValue(v))}</span>`)
    .join('');
  return meta;
}
